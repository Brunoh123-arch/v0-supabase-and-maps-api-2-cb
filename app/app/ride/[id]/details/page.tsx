  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      if (cancellationFee > 0) {
        alert(`Corrida cancelada. Taxa de cancelamento: R$ ${cancellationFee.toFixed(2)}`)
      } else {
          ...
          <Card className="p-6">
            ...
            {['accepted', 'on_way', 'in_progress'].includes(ride.status) && (
              <div className="mt-4 flex gap-2">
                <Button
                  onClick={() => router.push(`/app/ride/${ride.id}/chat`)}
        ...
        {['pending', 'accepted', 'on_way'].includes(ride.status) && (
          <Card className="p-6">
            ...
            <div className="space-y-3">
              {ride.status === 'pending' && (
                <Button
                  onClick={() => router.push(`/app/ride/${ride.id}/offers`)}
              ...
              {['accepted', 'on_way'].includes(ride.status) && (
                <Button
                  onClick={() => router.push(`/app/ride/${ride.id}/tracking`)}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white"
                >
                  Acompanhar Corrida
                </Button>
